# Packaging status — v0.1.8

This page tracks the state of the two install channels we ship in
v0.1.8: the `.mcpb` Desktop Extension (Claude Desktop) and the Yak
...
```
dist/
├── rhino-gh-mcp-0.1.8.mcpb                           ~280 KB
├── rhinogh-mcp-grasshopper-0.1.8-rh8_0-any.yak      ~600 KB
...
```
WARNING: Content version doesn't match manifest: '0.1.8+<sha>' != '0.1.8'
...
- The version mismatch is because `dotnet` appends the git commit SHA to
  ...
  The manifest version (`0.1.8`) is the source of truth that ships in
  Package Manager.
...
# Push both packages.
"$YAK" push dist/rhinogh-mcp-grasshopper-0.1.8-rh8_0-any.yak
