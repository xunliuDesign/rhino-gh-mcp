# Packaging status — v0.2.2

This page tracks the state of the two install channels we ship in
v0.2.2: the `.mcpb` Desktop Extension (Claude Desktop) and the Yak
...
```
dist/
├── rhino-gh-mcp-0.2.2.mcpb                           ~280 KB
├── rhinogh-mcp-grasshopper-0.2.2-rh8_0-any.yak      ~600 KB
...
```
WARNING: Content version doesn't match manifest: '0.2.2+<sha>' != '0.2.2'
...
- The version mismatch is because `dotnet` appends the git commit SHA to
  ...
  The manifest version (`0.2.2`) is the source of truth that ships in
  Package Manager.
...
# Push both packages.
"$YAK" push dist/rhinogh-mcp-grasshopper-0.2.2-rh8_0-any.yak
