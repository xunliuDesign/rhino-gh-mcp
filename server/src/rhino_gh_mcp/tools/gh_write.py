...
All tools register unconditionally; @gated checks current capabilities at
call time.
"""
...
def register(
    app: FastMCP, gh: GrasshopperBridge, caps: CapabilitiesProvider, config: Config
) -> None:
    """Register Grasshopper write tools."""
    ...
    @gated(caps, "gh_select_value_list")
    def gh_select_value_list(instance_guid: str, item: str) -> str:
        ...
        index exists, fall back to a name match.
    ...
    @gated(caps, "gh_set_component_parameter")
    ...
    def gh_add_component(
        ...
        Subject to the canvas-side ComponentScope: in CURATED scope, the .gha
        enforces the CategoryFilter allow-list; in DEFAULTS scope, only stock
        Grasshopper components are accepted; in ALL scope, anything goes.
    ...
    @gated(caps, "gh_connect_components")
    ...
    @gated(caps, "gh_add_any_component")
    def gh_add_any_component(
        component_name: str,
        position_x: int = 100,
        position_y: int = 100,
    ) -> str:
        """Place any component, bypassing the canvas-side CategoryFilter.
        ...
        Requires ComponentScope=ALL on the canvas-side component, otherwise
        the bridge will reject the request.
