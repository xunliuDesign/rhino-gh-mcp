...
These re-enable and extend the rhino_tools.py from _archive project, where the
tool registrations had been commented out. The wire protocol is preserved so
the existing rhino_script_client/MCPService.cs can keep serving them; new
commands (run_named_command, etc.) land alongside the Rhino plugin work.
"""
...
def register(app: FastMCP, rhino: RhinoBridge, policy: Policy) -> None:
    ...
    if _gate("rhino_get_objects_with_metadata"):
    ...
    if _gate("rhino_execute_code"):
        ...
        def rhino_execute_code(code: str) -> str:
            """Execute Python in Rhino (IronPython 2.7 in Rhino 7, CPython 3 in Rhino 8).
    ...
    if _gate("rhino_run_named_command"):
        ...
        def rhino_run_named_command(command: str, echo: bool = False) -> str:
            ...
            wrap with a typed tool. The plugin handles command-line marshalling
            and returns the command's exit status.

            NOTE: Plugin-side handler lands in P3; until then this returns an
