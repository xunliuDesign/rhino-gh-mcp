...
Cross-cutting bridge-version-specific behaviors that have bitten LLM
builds. These are NOT about Grasshopper itself — they're about the
...
| `Reparameterize` | `not found in ANY scope` | Not a standalone component — it's a right-click input option on Surface params | Right-click the Surface input → "Reparameterize" via the canvas-side UI (not exposed via the bridge for setting from a tool call). Use `Bounds` of the surface's actual domain instead, OR construct the (0, 1) domain manually with `Construct Domain` |
| `Merge` | `Component_MergeN` (Merge Multiple — dynamic Stream 0/1/...) | Auto-expanding inputs | Use `append=True` on subsequent wires to same stream to grow N |
...
1. **Only the first GUID is resolved** if you pass a newline-separated
   ...
   For N hosts, use N separate refs + `Merge Multiple` (see
   `host-ingest.md`).
...
## Expression component — VariantParameter requirement

The `Expression` component's default inputs `x` and `y` are
`VariantParameter` type (auto-coerce between Number / Vector / etc.).
If you add a third input via `gh_set_component_parameter` and it
comes back as `Param_GenericObject`, the Expression formula will
return `None` for that input. Fix: ensure new inputs on Expression are
created as `VariantParameter` (the same kind as the original two).
